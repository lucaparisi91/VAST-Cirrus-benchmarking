...
class PyUrllib3(PythonPackage):
    ...
    with default_args(deprecated=True):
        ...
        version(
            "1.26.20", sha256="40c2dc0c681e47eb8f90e7e27bf6ff7df2e677421fd46756da1161c39ca70d32"
        )
        version(
            "1.26.12", sha256="3fa96cf423e6987997fc326ae8df396db2a8b7c667747d47ddd8ecba91f4a74e"
        )
        version(
            "1.26.6", sha256="f57b4c16c62fa2760b7e3d97c35b255512fb6b59a259730f36ba32ce9f8e342f"
        )
        version(
            "1.25.11", sha256="8d7eaa5a82a1cac232164990f04874c594c9453ec55eef02eab885aa02fc17a2"
        )
        version(
            "1.25.9", sha256="3018294ebefce6572a474f0604c2021e33b3fd8006ecd11d62107a5d2a963527"
        )
        version(
            "1.25.6", sha256="9a107b99a5393caf59c7aa3c1249c16e6879447533d0887f4336dde834c7be86"
        )
        version(
            "1.25.3", sha256="dbe59173209418ae49d485b87d1681aefa36252ee85884c31346debd19463232"
        )
        version(
            "1.24.3", sha256="2393a695cd12afedd0dcb26fe5d50d0cf248e5a66f75dbd89a3d4eb333a61af4"
        )
        version(
            "1.21.1", sha256="b14486978518ca0901a76ba973d7821047409d7f726f22156b24e83fd71382a5"
        )
    ...
    with default_args(type="build"):
        depends_on("py-hatchling@1.27:1", when="@2.7:")
        depends_on("py-hatchling@1.6:1", when="@2:2.6")
        depends_on("py-hatch-vcs@0.4:0.5", when="@2.5:")
        depends_on("py-hatch-vcs@0.4", when="@2.3:2.4")
        depends_on("py-setuptools-scm@8:10", when="@2.7:")
        depends_on("py-setuptools-scm@8", when="@2.5:2.6")
    ...
    with default_args(type="run"):
        with when("+brotli"):
            depends_on("py-brotli@1.2:", when="@2.6:")
            depends_on("py-brotli@1.0.9:", when="@1.26.9:")

            # Historical dependencies
            depends_on("py-brotlipy@0.6:", when="@:1.26.8")

        with when("+socks"):
            depends_on("py-pysocks@1.5.6,1.5.8:1")

        # Historical dependencies
        with when("+secure"):
            depends_on("py-pyopenssl@17.1:", when="@2:")
            depends_on("py-pyopenssl@0.14:", when="@1")
            depends_on("py-cryptography@1.9:", when="@2:")
            depends_on("py-cryptography@1.3.4:", when="@1")
            depends_on("py-idna@2:")
            depends_on("py-certifi")
            depends_on("py-urllib3-secure-extra", when="@1.26.12:")